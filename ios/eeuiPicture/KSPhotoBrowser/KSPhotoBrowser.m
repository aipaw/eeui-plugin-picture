...
- (void)removeCurrentPage {
    if (self.removeCallback) {
        self.removeCallback(self.currentPage);
    }
    
    NSMutableArray *list = [NSMutableArray arrayWithArray:self.photoItems];
    [list removeObjectAtIndex:self.currentPage];
    self.photoItems = list;
    
    if (list.count == 0) {
        [self dismissViewControllerAnimated:NO completion:nil];
    } else {
        NSInteger num = self.currentPage;
        ...
        self.scrollView.contentSize = CGSizeMake(self.scrollView.frame.size.width * self.photoItems.count, 0);
        ...
        self.visibleItemViews = [[NSMutableArray alloc] init];
